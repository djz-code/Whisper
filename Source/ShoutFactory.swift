...
public func Shout(announcement: Announcement, to: UIViewController, completion: (() -> ())? = {}) {
  shout.craft(announcement: announcement, to: to, completion: completion)
}
...
public class ShoutView: UIView {
  ...
  public private(set) var displayTimer = Timer()
...
//    Dimensions.height = UIApplication.sharedApplication().statusBarHidden ? 70 : 80
    ...
    shouldSilent = false
    configureView(announcement: announcement)
    shout(to: to)
  ...
  public func configureView(announcement: Announcement) {
    ...
    displayTimer = Timer.scheduledTimerWithTimeInterval(announcement.duration,
...
//    }
  ...
  public func setupFrames() {
    ...
    let imageYOffset: CGFloat = UIApplication.shared
        .isStatusBarHidden ? 15 : 25
    ...
    Dimensions.height = max(UIApplication.shared.isStatusBarHidden ? 70 : 80, titleLabel.frame.origin.y + 15 + titleLabel.frame.size.height + subtitleLabel.frame.size.height)
